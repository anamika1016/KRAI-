...
ActiveRecord::Schema[8.1].define(version: 2026_09_24_133000) do
  ...
  create_table "module_records", force: :cascade do |t|
    ...
    t.index "(((data)::jsonb ->> 'record_id'::text))", name: "index_training_edit_requests_on_pending_record_id", where: "(((module_slug)::text = 'training-form-edit-request'::text) AND (((data)::jsonb ->> 'status'::text) = 'Pending'::text))"
    ...
    t.index "COALESCE(NULLIF(((data)::jsonb ->> 'vrp_id'::text), ''::text),\nCASE\n    WHEN (lower(((data)::jsonb ->> 'created_by_record_type'::text)) = 'vrp'::text) THEN ((data)::jsonb ->> 'created_by_id'::text)\n    ELSE NULL::text\nEND), lower(btrim(((data)::jsonb ->> 'month'::text))), lower(btrim(((data)::jsonb ->> 'main_activity'::text)))", name: "index_training_forms_on_owner_month_activity", where: "((module_slug)::text = 'training-form'::text)"
    ...
    t.index "lower(btrim(((data)::jsonb ->> 'month'::text)))", name: "index_module_records_on_normalized_month_all"
    ...
    t.index "lower(btrim(((data)::jsonb ->> 'month'::text))), lower(btrim(((data)::jsonb ->> 'main_activity'::text))), lower(btrim(((data)::jsonb ->> 'fco_name'::text)))", name: "index_module_records_on_month_activity_fco"
    t.index "lower(btrim(((data)::jsonb ->> 'month'::text))), lower(btrim(COALESCE(((data)::jsonb ->> 'main_activity'::text), ''::text)))", name: "index_training_forms_on_month_and_main_activity", where: "((module_slug)::text = 'training-form'::text)"
    ...
    t.index ["id"], name: "index_training_edit_requests_on_pending_id", order: :desc, where: "(((module_slug)::text = 'training-form-edit-request'::text) AND (((data)::jsonb ->> 'status'::text) = 'Pending'::text))"
    t.index ["module_slug", "created_at"], name: "index_module_records_on_slug_and_created_at", order: { created_at: :desc }
    t.index ["module_slug", "id"], name: "index_training_edit_requests_on_slug_and_id", order: { id: :desc }, where: "((module_slug)::text = 'training-form-edit-request'::text)"
