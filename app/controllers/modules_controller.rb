...
class ModulesController < ApplicationController
  ...
  def dashboard
    ...
    @ics_farmer_report_summary = ics_farmer_report_summary(
      @ics_farmer_report_rows,
      selected_ics: @ics_farmer_report_selected_ics
    )
  ...
  def weekly_activity_target_status_cards(targets, month_name: nil, fcoc_name: nil, week_number: nil)
    ...
    [
      ...
      },
      {
        ...
        caption: "Kuch trainings complete aur kuch pending.",
      ...
      },
      {
        ...
        caption: "Kisi mapped training me completion nahi hua.",
  ...
  def training_participation_dashboard_counts(month_name:, fcoc_name:, records:)
    ...
    counts = {
      ...
      registered_farmer_total: training_registered_afl_farmer_count_for_participation(targets),
  ...
  def training_registered_afl_farmer_count_for_participation(targets)
    return 0 unless model_ready?(:Afl)

    vrp_afl_farmer_count(
      targets,
      targets: targets,
      vrps: Array(targets).filter_map(&:vrp)
    )
  end
  ...
  def training_record_completed_activity_keys_for_target(record, target)
    ...
    if matched_entries.blank? && target_entries.one? && training_record_matches_dashboard_target?(record, target, target_farmer_ids(target))
  ...
  def training_participation_status_caption(status)
    {
      ...
      "unique" => "Selected month me farmer training se mapped unique farmers.",
      "training_unique" => "Mapped farmers me kam se kam ek training complete.",
